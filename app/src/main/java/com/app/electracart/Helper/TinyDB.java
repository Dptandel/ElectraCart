...
public class TinyDB {
    ...
    public <T> ArrayList<T> getListObject(String key, Class<T> clazz) {
        Gson gson = new Gson();
        String json = getString(key); // Get the saved JSON string
        if (json == null) return new ArrayList<>(); // Return empty list if null
        Type type = TypeToken.getParameterized(ArrayList.class, clazz).getType();
        return gson.fromJson(json, type); // Deserialize JSON to ArrayList<T>
    }
    ...
    public void putListObject(String key, ArrayList<?> list) {
        Gson gson = new Gson();
        String json = gson.toJson(list); // Convert list to JSON array
        putString(key, json); // Save as a string
    }
